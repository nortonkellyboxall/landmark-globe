...
const _projToCam = new THREE.Vector3();
const _projOutward = new THREE.Vector3();
...
function projectEarthLatLng(lat, lng, alt = 0.02) {
  ...
  const toCam = _projToCam.copy(_projWorld).sub(camera.position).normalize();
  const outward = _projOutward.copy(_projWorld).sub(bodyWorldPos("earth")).normalize();

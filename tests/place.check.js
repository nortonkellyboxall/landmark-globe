...
assert.ok(LANDMARKS.find((p) => p.id === "stonehenge"));
assert.ok(LANDMARKS.find((p) => p.id === "chichenitza"));
assert.equal(placeById("stonehenge")?.continent, "Europe");
assert.equal(placeById("chichenitza")?.continent, "Americas");
assert.ok(placeById("chichenitza")?.lat >= 7);

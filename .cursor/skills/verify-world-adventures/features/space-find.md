...
- **Resolve target + assert not ISS.** Run `control-world-adventures browser eval --js '(()=>{const hit=document.querySelector("#ssSizesRow .ss-size-item.find-target");const id=hit?.getAttribute("data-id")||null;const strip=id?document.querySelector("#strip .thumb[data-id=\\""+id+"\\"]"):null;return {id, hasFindTarget:!!hit, stripHasIss:!!document.querySelector("#strip .thumb[data-id=\\"iss\\"]"), sizesIss:document.querySelectorAll("#ssSizesRow [data-id=\\"iss\\"]").length, spaceMode:document.body.classList.contains("space-mode")};})()'`. Expect a non-null body `id` that is not `iss`, `hasFindTarget` `true`, `stripHasIss` `false`, `sizesIss` `0`, `spaceMode` `true`.
...
- **Assert still Space.** Run `control-world-adventures browser eval --js '({spaceMode:document.body.classList.contains("space-mode"), solarHidden:document.getElementById("solarSystem").hidden, selected:document.getElementById("tabSpace").getAttribute("aria-selected")})'`. Expect `spaceMode` `true`, `solarHidden` `false`, `selected` `true`.

...
- **Find another.** Choose Find another. Run `control-world-adventures browser click --role button --name "Find another"`. Cue returns to `Find this!` with a new (or remaining) target while `#findPrompt` stays open.
- **Proof win.** Before Find another, or after a second win, run `control-world-adventures browser snapshot --aria --path find-quiz/win.aria.txt` and `control-world-adventures browser screenshot --path find-quiz/win.png`. The snapshot shows the found cue and an enabled Find another control.
